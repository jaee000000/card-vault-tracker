...
## PRICE is web-search-FIRST on every scan; DB prices are only a fallback
By explicit user request ("use web search for prices all the time first on every card scanned... successful every time"), lookupCard now kicks off `webSearchRawPriceGBP` ONCE at the top as `livePricePromise` (fired before Phase 0, runs concurrently with the image lookup). A single `finalize(dbPrice, imageUrl, dbPsa10GBP?)` helper awaits it and ALWAYS prefers the live web price; the DB price (PokéTCG/PriceCharting `bestPrice`) is used only when the live price is 0. EVERY return path routes through finalize — Phases 0 & 4 (PriceCharting, pass pc.psa10GBP), 1/2/3 (English twin), 5 (now IMAGE-only: exact twin art else webSearchOfficialImage), 6 (image-only). priceNote: live>0 → "Price from live web search (sold listings)"; else db>0 → jpNote (JP) / null (EN); else "Japanese card — not in price database" (JP) / null.
**Why:** prices were failing ("Price N/A") or coming from a wrong/over-priced English twin; the live sold-listing search is the most reliable per-card source and reaches the open web from prod (PriceCharting is blocked there). **Tradeoff the user accepted:** finalize always awaits the web call, so EVERY scan (incl. English cards PokéTCG could price instantly) now waits ~up to 20s — bounded by webSearchRawPriceGBP's 20s AbortSignal timeout, and overlapped with the image lookup so it doesn't stack.
**Watch:** (1) any NEW return path must go through finalize or it bypasses web-first. (2) webSearchRawPriceGBP runs for English cards too now — its prompt only says "Japanese-language" when `isJapaneseSet(setId)`. (3) webSearchRawPriceGBP catches all errors and returns 0, so awaiting livePricePromise can't reject. Real cases that drove this: Rayquaza M2A 127/193 (£21.93 wrong→£1.58), Sliggoo 093/83 M4 AR (N/A→£3.32).
...
## Scan latency / "Network error" — keep /scan/identify well under the gateway timeout
The frontend's only failure path is a catch-all "SCAN FAILED — Network error" shown whenever /api/scan/identify doesn't return parseable JSON — i.e. the connection/gateway timed out (autoscale cold start + a long request). So scan reliability == keeping the request SHORT, not adding client retries.
Two things were silently stacking ~20s each onto JP scans: (a) the web-first price search (`webSearchRawPriceGBP`, 20s AbortSignal) being awaited directly in finalize, and (b) phases 5 & 6 running `webSearchOfficialImage` (another ~20s) EVEN FOR JP cards whose art already comes from the concurrent LimitlessTCG lookup (jpImagePromise) and is overridden in finalize anyway — pure dead weight.
**Fix:** (1) finalize awaits `livePriceCapped = Promise.race([livePricePromise, 12s])` (timer unref'd + cleared) instead of the raw promise. (2) phases 5/6 skip webSearchOfficialImage when `jpSet` (pass null; finalize uses limitless), and phase 5's return condition includes `jpSet` so JP cards always finalize. Result: JP scan ~13-20s → ~6s.
**Price guarantee preserved:** the cap can return price 0 only when the search is genuinely slow (rare). POST /api/cards now re-fetches when the incoming `currentPriceGBP` is 0/missing (was `currentPriceGBP ?? fetch`, which kept a 0) via `fetchLivePriceGBP`, so a saved card reliably ends with a price even if the scan capped out.
**Watch:** don't reintroduce a direct `await livePricePromise`/`webSearchOfficialImage` on a JP path; any new return path must keep the request bounded. The 12s cap assumes the web search usually finishes in ~2-4s (it does).

## Detail-panel "art swaps to the wrong card on open" + fullscreen crash
Two bugs from the same area. (1) WRONG ART: the card detail panel auto-"heals" art by calling GET /cards/:id/hires-image whenever the stored image isn't a recognised art host. The client's proper-art check AND the server's short-circuit only listed pokemontcg/scrydex/pricecharting — NOT the LimitlessTCG hosts — so a JP card already holding correct limitless art was re-resolved, and the server's `tcgHiResLookup` (PokéTCG `name:"X" number:N`, NO set constraint) matched a wrong same-number English card and OVERWROTE the good art in the DB. Number alone is not a set constraint — many English sets share a number.
**Fix:** keep ONE shared proper-art host list (must include `limitlesstcg.com` AND `limitlesstcg.nyc3.cdn.digitaloceanspaces.com`) on BOTH sides. Server hires-image short-circuits (returns stored URL, no DB write) for ANY proper-art host = idempotent. For JP sets resolve via set-scoped `limitlessJpImageUrl` FIRST and never use `tcgHiResLookup` (English-only, unsafe for JP). Verified: proper-art card returns same URL in ~10-30ms.
**Watch:** any new art-host CDN must be added to the proper-art list in ALL places or the heal loop will fight it (re-resolve → overwrite → re-render).
(2) FULLSCREEN CRASH on mobile: `useHoloTilt` runs a ~60fps `setState` rAF loop FOREVER (idle sway never stops), re-rendering the panel continuously; opening the fullscreen portal (a second full-size image decode) on top of that sustained churn killed the tab. Plus rendering the user's raw camera `data:` URI (≈12MP) at full size spikes memory.
**Fix:** `useHoloTilt(maxAngle, perspectivePx, enabled)` — panel passes `!fullscreen` so the loop pauses while fullscreen is open. Fullscreen viewer skips the hires re-fetch for already-proper art, and for a non-proper (data:URI) image it withholds rendering until the server resolves real, correctly-sized art (shows a spinner) instead of decoding the giant photo.
