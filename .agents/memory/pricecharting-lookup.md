...
## NEVER pick a card IMAGE by closest set total — only exact name+number
The displayed thumbnail must come from an EXACT name + EXACT number twin. The old Phase 5 image selection fell back to `sortBySetTotal(exactName)[0]` when no exact-number twin existed, which for a Japanese card grabbed a DIFFERENT English card of the same Pokémon whose printedTotal was merely closest (e.g. JP Rayquaza M2A 127/193 → showed Vivid Voltage "Amazing Burst" Rayquaza, total 185 ≈ 193). PRICE may use a set-total estimate (it's just an approximation), but the IMAGE must not. If no exact-number twin, leave imageUrl null and let webSearchOfficialImage fetch the real art.
**Why:** user scanned Rayquaza M2A 127/193, identified correctly but wrong artwork shown. **How to apply:** any image-from-English-equivalent path (lookupCard Phase 5, findCardImage, tcgHiResLookup) must require exact number match; tie-break among exact-number matches by closest total, never a loose total-only fallback.
